...
export default function TicketsPage() {
  ...
  const filtered = tickets.filter(t =>
    [t.customerName, t.deviceModel, t.tokenId, t.status].some(f => f.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 border-b bg-background/80 backdrop-blur-md">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">RepairLab Tickets</span>
          </div>
          <div className="flex items-center gap-2">
            <NewTicketDialog />
            <InviteAdminDialog />
      ...
      <main className="mx-auto max-w-6xl space-y-4 p-4">
        <div className="animate-reveal-up flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="animate-reveal-up-delay-1 surface-elevated overflow-hidden rounded-xl border">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="hidden px-4 py-3 text-left font-medium text-muted-foreground sm:table-cell">Device</th>
                  <th className="px-4 py-3 text-left font-medium text-muted-foreground">Status</th>
                  <th className="hidden px-4 py-3 text-left font-medium text-muted-foreground md:table-cell">Received</th>
                ...
                {loading ? (
                  ...
                  filtered.map(ticket => (
                    <tr
                      ...
                      className="cursor-pointer border-b transition-colors last:border-0 hover:bg-muted/40 active:scale-[0.995]"
                    >
                      ...
                      <td className="hidden px-4 py-3 text-muted-foreground sm:table-cell">{ticket.deviceModel}</td>
                      <td className="px-4 py-3">
                        <StatusBadge status={ticket.status} />
                      </td>
                      <td className="hidden px-4 py-3 font-mono text-xs text-muted-foreground md:table-cell">{ticket.dateReceived}</td>
                      <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                        <Button
                          size="sm"
                          variant="destructive"
